package cn.linkey.rulelib.KsfDemo1;
 ...
 * @author  
 * @version: 8.0
 * @Created: 2018-10-24 10:09:43
 */
final public class R_KsfDemo1_B001 implements LinkeyRule {
	@Override
    ...
    /** todo 获取原始的上传文件名称 */
    private String getAttFileName(String fileIDName){
        String fileID="",fileName="";
        fileID = fileIDName.substring(0,fileIDName.lastIndexOf("."));
        
        String sql="select FileName from BPM_AttachmentsList where wf_orunid='"+fileID+"'";
        fileName=Rdb.getValueBySql(sql);
    ...
    public static Boolean zip(List<File> fileList, File saveDir, String zipFileName, Boolean cover) {
        ...
        try {
            ...
            for (File item : fileList) {
                ...
                compress(zos, item, zipFileName+File.separator); // 如果不想放在一个文件夹里面，那么替换 zipFileName+File.separator 为 "" 即可
            }
            zos.flush();
            zos.close();
            return true;
